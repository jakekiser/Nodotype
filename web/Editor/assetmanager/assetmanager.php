...
if(isset($_FILES["File1"]))
  ...
  if($MaxFileSize && ($_FILES['File1']['size'] > $MaxFileSize))
    ...
    // Add to XPage Web file system
    $sFileName=str_replace('\'','\'\'',$sUploadedFile);
    $sRelativePath=str_replace('\'','\'\'',$sBaseVirtual0);
    $iFileType=str_replace('\'','\'\'',0);
    $sDescription=str_replace('\'','\'\'','');
    $iFileId=str_replace('\'','\'\'','');
    $FileSize=0;
    $SQL='call spDAInsUpdFile (\''.$iTennantId.'\',\''.$Persist_UserId.'\',\''.$sFileName.'\',\''.$sRelativePath.'\',\''.$sDescription.'\',\''.$iFileType.'\',\''.$iFileId.'\',\''.$FileSize.'\');';
...
if(isset($_POST["inpFileToDelete"]))
  ...
  if($filename!="") {
    ...
    // Delete from XPage Web file system
    $SQL='call spDADelFileByName (\''.$iTennantId.'\',\''.$UserId.'\',\''.$filename.'\',\'0\');';
    $result = mysql_query($SQL);  
  }
  $sMsg = "";
 }
...
<td valign=top style="height:100%;background:url('bg.gif') no-repeat right bottom;padding-top:5px;padding-left:5px;padding-right:5px;padding-bottom:0px;">
    ...
    <td>
        ...
        <td nowrap>
          <span style="display:none;" onclick="newFolder()" style="cursor:pointer;"><u><span name="txtLang" id="txtLang" <?php echo $sFolderAdmin;?>>New&nbsp;Folder</span></u></span>&nbsp;
          <span style="display:none;" onclick="deleteFolder()" style="cursor:pointer;"><u><span name="txtLang" id="txtLang" <?php echo $sFolderAdmin;?>>Del&nbsp;Folder</span></u></span>
        </td>
